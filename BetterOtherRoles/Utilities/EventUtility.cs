...
public static class EventUtility
{
    ...
    {
        if (!isEnabled || eventQueue == null ||
            AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started ||
            BetterOtherRoles.Rnd == null || IntroCutscene.Instance) return;
    ...
    public static bool isEnabled => isEventDate || canBeEnabled && CustomOptionHolder.enableCodenameHorsemode != null &&
    ...
    public static void meetingStartsUpdate()
    {
        ...
        {
            AddToQueue(EventTypes.Communication);
        }
        ...
        HudManager.Instance.StartCoroutine(Effects.Lerp(1f, new Action<float>((p) =>
        {
            if (MeetingHud.Instance && MeetingHud.Instance.playerStates != null)
            {
                ...
                {
                    GameData.PlayerInfo pInfo = GameData.Instance.AllPlayers.ToArray()
                        .First(x => x.PlayerId == pva.TargetPlayerId);
    ...
    public static void gameStartsUpdate()
    {
        ...
        meetingEndsUpdate();
        List<CachedPlayer> relevantPlayers =
            CachedPlayer.AllPlayers.Where(x => !x.Data.IsDead && x != CachedPlayer.LocalPlayer).ToList();
