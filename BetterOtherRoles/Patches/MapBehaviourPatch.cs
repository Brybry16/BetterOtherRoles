...
namespace BetterOtherRoles.Patches
{
	...
	static class MapBehaviourPatch
	{
		...
		public static Sprite Vent = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Vent.png", 150f);
		...
		static void Postfix(MapBehaviour __instance)
		{
			...
			if (Trapper.trapper != null && CachedPlayer.LocalPlayer.PlayerId == Trapper.trapper.PlayerId)
			{
				foreach (PlayerControl player in Trapper.playersOnMap)
				{
					...
					v.z = -2.1f;
					var herePoint = UnityEngine.Object.Instantiate(__instance.HerePoint,
						__instance.HerePoint.transform.parent, true);
			...
			foreach (var vent in MapUtilities.CachedShipStatus.AllVents)
			{
				...
				{
					MapIcon = GameObject.Instantiate(__instance.HerePoint.gameObject,
						__instance.HerePoint.transform.parent);
				...
				if (AllVentsRegistered(Instance))
				{
					...
					foreach (var connectedgroup in VentNetworks)
					{
						...
						if (connectedgroup[0].name.StartsWith("JackInThe"))
							continue;
						connectedgroup.Do(x =>
							GetIcon(x).GetComponent<SpriteRenderer>().color = Palette.PlayerColors[index]);
					}
